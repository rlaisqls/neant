...
pub fn strongest_first(bounds: &mut Vec<Bound>, m: &Machine) {
    ...
    let at = |p: &Poly| p.eval(&|a| match a { Atom::B => Some(m.b_bytes as f64), Atom::M => Some(m.m_bytes as f64), Atom::P => Some(m.p_cores as f64), Atom::Var(_) => Some(1e6), Atom::Log(_) | Atom::Opaque(_) | Atom::Read(_) => None }).unwrap_or(0.0);
