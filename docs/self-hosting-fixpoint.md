...
```
stage1   compiler/*.nt concatenated — four stages and main.nt — run by the Rust interpreter
stage1 < stage1's source  ─▶ stage2.c   (182,390 bytes, and this is the committed bootstrap/neant.c)
cc stage2.c rt.c          ─▶ stage2     (a native binary)
stage2 < stage1's source  ─▶ C          ─── byte-identical to stage2.c
```

`compiler/build.sh` does exactly this and writes `bootstrap/neant.c`.
...
Seed two is now checked in: `bootstrap/neant.c`, 182,390 bytes, regenerated by `compiler/build.sh` and
compared against the live compiler's output by the fixpoint test — a checked-in artifact that
nothing checks is a file that silently stops matching its source.

What it took was `compiler/main.nt`, the fifth fragment: a committed driver instead of one a test
assembles with paths compiled in. It is a **filter** — `./neant-self < x.nt > x.c` — because the
language has no argv and giving it one would mean the emitted `main` taking parameters and carrying
them across a translation unit for every program, wanted or not. `read_stdin` and `write_stdout`
joined `read_file`/`write_file` in `rt.c`, on the same pointer-and-length convention, and so did
`quit`: libc's `exit` cannot be named by an `extern fn` at all, because neant's `i64` is `int64_t`
where libc's parameter is `int`, and two declarations of `exit` that disagree is something `cc`
refuses. One line of shim, rather than an integer-width rule in the language.

The concatenation order — lex, parse, check, emit, main — is the whole build system, and it is the
dependency order: there is no module system, so these are fragments of one program.
