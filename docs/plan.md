...
this compiler does not emit. **`moves`'s first slice landed too**: 26 columns exact for functions that call nothing, by the rule
...
refills and no wall-clock change. The estimate improved; the program did not. Regimes, recurrences, span are still out — nor are M5's moves and uniqueness (the emitter takes the safe branch and copies),
