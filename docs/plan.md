...
this compiler does not emit. **`moves`'s first slice landed too**: 25 columns exact for functions that call nothing, by the rule
...
design before a line was written. One difference, `vm.nt`'s `run`, nobody has explained, and it is
listed as such rather than excused. Regimes, recurrences, span are still out — nor are M5's moves and uniqueness (the emitter takes the safe branch and copies),
