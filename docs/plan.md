...
**70 columns exact with 3 declined**, the declines being the `main`s whose callee leaves a footprint
per *field* resident under SoA. A cost became a list of pieces rather than one fork, and then the
...
in. Five of the six things that then had to be right were invisible until a column carried two
...
bisection, which is strictly stronger than comparing them symbolically. Recurrences and span are
still out — nor are M5's moves and uniqueness (the emitter takes the safe branch and copies),
